...
p <- allProbs %>%
  ...
  ungroup() %>%
  pivot_longer() %>%
  ggplot(aes(x = reorder(problem, metric), y = metric)) +
...
However, this report will focus on a single dataset - MoteStrain.

A visual check of the raw time series is useful to gauge any statistical peculiarities or issues with the import. A random sample of ten time series - five from the *train* class and five from the *test* - is presented in Figure \@ref(fig:tsplot).

```{r tsplot, warning = FALSE, message = FALSE, fig.keep = TRUE, fig.cap = "Random subset of ten raw time series"}
# Filter to just MoteStrain and free up memory from the main dataset
...
p1 <- MoteStrainFilt %>%
  ...
  ggplot(aes(x = timepoint, y = value, colour = target)) +
  geom_line() +
  labs(x = "Timepoint",
       y = "Value") +
