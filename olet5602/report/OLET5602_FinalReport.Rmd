...
for(i in 1:length(models)){
  ...
  pred <- predict(models[[i]], newdata = pred_data, probability = TRUE, decision.values = TRUE) # Generate predictions
  cm <- table(pred, eval_dfs[[i]]$label) # Confusion matrix
  
  results <- data.frame(Model = names(model)[i],
                        Accuracy = acc,
                        `Balanced Accuracy` = bal_acc,
                        Sensitivity = sens,
                        Specificity = spec) # Summarise data
