...
osn_pca_plot <- osn_pca_df %>%
  ...
  stat_ellipse(data = subset(osn_pca_df, label == "OSN"), geom = "polygon", alpha = 0.4, fill = "#F8766D") +
...
# Find "Other" genes which are further than 1.96 * SD

other <- osn_pca_df %>%
  filter(label == "Other") %>%
  dplyr::select(c(gene, .fittedPC1, .fittedPC2)) %>%
  mutate(flag = case_when(
          .fittedPC1 > (mu[1] + 1.96 * sigma[1]) & .fittedPC2 > (mu[2] + 1.96 * sigma[2]) ~ TRUE,
          .fittedPC1 < (mu[1] - 1.96 * sigma[1]) & .fittedPC2 < (mu[2] - 1.96 * sigma[2]) ~ TRUE,
          .fittedPC1 > (mu[1] + 1.96 * sigma[1]) & .fittedPC2 < (mu[2] - 1.96 * sigma[2]) ~ TRUE,
          .fittedPC1 < (mu[1] - 1.96 * sigma[1]) & .fittedPC2 > (mu[2] + 1.96 * sigma[2]) ~ TRUE,
          TRUE                                                                            ~ FALSE)) %>%
...
osn_pca_plot_hl <- osn_pca_plot_hl %>%
  ...
  stat_ellipse(data = subset(osn_pca_plot_hl, label == "OSN"), geom = "polygon", alpha = 0.4, fill = "#F8766D") +
...
Preparation of the epigenome datasets was also performed separately for each of the six histone marks:
...
The model that was chosen is a XX.

```{r, message = FALSE, warning = FALSE}
library(caret)

#' Function to create a train-test split and do 1-fold CV
#' 
...
fit_model <- function(data, model = "gaussprRadial", seed){
  
  set.seed(seed)
  
  # Create train-test sets
  
  inTrain <- caret::createDataPartition(y = X2$label, p = .75, list = FALSE)
  train <- X2[inTrain, ]
  test <- X2[- inTrain, ]
