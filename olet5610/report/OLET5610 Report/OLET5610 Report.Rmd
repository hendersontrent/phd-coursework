...
See Figure \@ref(fig:impplots)).

```{r impplots, fig.cap = "Variable importance summaries over 30 resamples from random forest classification models. (A) Frequency of ranks over all resamples are plotted for each principal component used as a predictor in the models. $PC 2$ demonstrates the most first rankings for variable importance. (B) Mean variable importance $\pm$ 1SD is plotted for each principal component used as predictors in the models.", echo = FALSE, warning = FALSE, message = FALSE}
...
imp_ranks <- importances %>%
  ...
  geom_bar(stat = 'identity', fill = "#1B9E77", alpha = 0.9) +
  labs(title = "(A) Frequency of variable importance ranks over all resamples",
       x = "Rank",
       y = "Frequency") +
  ggplot2::theme_bw() +
  ggplot2::theme(panel.grid.minor = ggplot2::element_blank(),
...
imp_dists <- importances %>%
  ...
  geom_errorbar(aes(ymin = lower, ymax = upper), colour = "#D95F02", size = 1) +
  geom_point(colour = "#D95F02", size = 2) +
  labs(title = "(B) Distribution of variable importance values over all resamples",
       x = "Principal component",
...
importance_plots <- ggpubr::ggarrange(imp_ranks, imp_dists, nrow = 2, ncol = 1, common.legend = TRUE, legend = "bottom")
